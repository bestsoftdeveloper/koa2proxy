...
const { matchPath } = require('match-path-plus');
/**
 * Constants
 */
 ...
 */
exports.httpProxy = (urlPattern, options) => (ctx, next) => {
    if (!matchPath(urlPattern, ctx.req.url))
        return next();
    let opts = Object.assign({}, options);
    if (typeof options === 'function') {
        const { params, path } = matchPath(urlPattern, ctx.req.url);
